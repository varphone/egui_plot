...
impl PlotItem for ArcLine {
    fn shapes(&self, _ui: &Ui, transform: &PlotTransform, shapes: &mut Vec<Shape>) {
        ...
        let max_x_pos = transform.position_from_point(&PlotPoint::new(self.center.x + self.radius, self.center.y));
...
impl PlotItem for Pie {
    fn shapes(&self, _ui: &Ui, transform: &PlotTransform, shapes: &mut Vec<Shape>) {
        ...
        shapes.push(Shape::pie(center, radius, start_angle, end_angle, fill, stroke));
    }
    ...
    fn find_closest(&self, point: Pos2, transform: &PlotTransform) -> Option<ClosestElem> {
        ...
        if contains_in_pie(center, radius, self.start_angle, self.end_angle, point) {
            return Some(ClosestElem { index: 0, dist_sq: 0.0 });
        }
        // }
    ...
    fn on_hover(
        ...
        plot.ui.fonts_mut(|f| {
            ...
            shapes.push(Shape::text(
                f,
                plot.transform.position_from_point(&PlotPoint::new(center.x, center.y)),
...
impl PieChart {
    ...
    pub fn new(name: impl Into<String>, center: impl Into<PlotPoint>, radius: f64, data: Vec<f64>) -> Self {
        Self {
            base: PlotItemBase::new(name.into()),
            center: center.into(),
            radius,
...
impl PlotItem for PieChart {
    fn shapes(&self, ui: &Ui, transform: &PlotTransform, shapes: &mut Vec<Shape>) {
        ...
        for pie in pies {
            pie.highlight(self.base.highlight).shapes(ui, transform, shapes);
        }
    }
    ...
    fn find_closest(&self, point: Pos2, transform: &PlotTransform) -> Option<ClosestElem> {
        ...
        for (i, (start, end)) in angle_pairs.into_iter().enumerate() {
            if contains_in_pie(center, radius, start, end, point) {
                return Some(ClosestElem { index: i, dist_sq: 0.0 });
            }
        }
    ...
    fn on_hover(
        ...
        plot.ui.fonts_mut(|f| {
            ...
            let center = center_of_pie(self.center.to_pos2(), self.radius as f32, start_angle, end_angle);
...
fn calculate_arc_bounds(center: PlotPoint, radius: f64, start_angle: f32, end_angle: f32) -> PlotBounds {
    let x = center.x;
    let y = center.y;
    let r = radius;
...
fn contains_in_pie(center: Pos2, radius: f32, start_angle: f32, end_angle: f32, point: Pos2) -> bool {
